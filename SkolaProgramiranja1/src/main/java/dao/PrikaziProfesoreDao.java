...
public class PrikaziProfesoreDao {
	...
	public List<ProfesorWrapper>vratiSveProfesore() {
		...
		List<ProfesorWrapper>listaProfesora = new ArrayList<ProfesorWrapper>();
		...
		try {
			...
			System.out.println("Lista ne moze da se vrati!" + e);
			return null;
	...
	public void dodajPredmetProfesoru(String idProfesor, String idPredmet) {
		...
		try {
			...
			session.getTransaction().commit();
			System.out.println("Uspesno dodat predmet profesoru...");
		} catch (Exception e) {
			session.getTransaction().rollback();
